@extends('admin.mater')
@section('namepage',"Cập nhật mượn trả")
@section('content')
    <div class="col-md-12 col-sm-12 col-xs-12">
        @include('admin.block.alert')
        <div class="x_panel">
            <div class="x_title">
                <h2>Cập Nhật Mượn Trả </h2>
            ...
            <div class="x_content">
                <br />
                <form  action="{!! route('admin.muontra.postedit',['id'=>$muontra['id']]) !!}"  class="form-horizontal form-label-left" method="post">
                    {!! csrf_field() !!}
                            ...
                            <select name="sothe" class="form-control">
                                ...
                                @foreach($reader as $item)
                                    @if(date('Y/m/d',strtotime($item['thuvien']['ngayhethan']))>date('Y/m/d',strtotime(\Carbon\Carbon::now())))
                                        <option value="{!! $item['thuvien']['id'] !!}" {!! $item['thuvien']['id']==$muontra['sothe'] ?"selected":null !!}>{!! $item['tendocgia'] !!}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select name="masach" class="form-control">
                                ...
                                @foreach($book as $item)
                                    <option value="{!! $item['id'] !!}" {!! $item['id']==$muontra['ctmuontra']['masach'] ?"selected":null !!}>{!! $item['tensach'] !!}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input type="date" min="{!! $muontra['ngaymuon'] !!}" value="{!! date('Y-m-d',strtotime($muontra['ctmuontra']['ngaytra'])) !!}" name="ngaytra"  class="form-control col-md-7 col-xs-12">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <textarea style="resize: none"  rows="4" cols="50" name="ghichu" class="form-control col-md-7 col-xs-12">
                                {!! $muontra['ctmuontra']['ghichu'] !!}
                            </textarea>
                        </div>
                    </div>
